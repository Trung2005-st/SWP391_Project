...
public class RecommendPlanServiceImpl implements RecommendPlanService {
    ...
    public PlanDto recommendByAssessment(AssessmentRequestDto request) {
        ...
        QuitPlanDefinition best = repository
                .findByDifficultyLevelLessThanEqualOrderByDifficultyLevelAsc(desiredDifficulty)
                .stream()
                .max(Comparator.comparingInt(QuitPlanDefinition::getDurationDays))
                .orElseThrow(() -> new RuntimeException("No plan matches assessment"));
